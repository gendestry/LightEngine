...
void StaticColorGrad::apply(Engine::Frame &frame, const Engine::TimeContext &)
{
    ...
    // one colour stop per fixture, evenly ramped start -> end across the group
    const std::vector<Utils::Colors::RGB> colors = Utils::Colors::gradient(
        m_startColor, m_endColor, static_cast<uint16_t>(fids.size()));

    for (std::size_t i = 0; i < fids.size(); ++i)
    {
        Engine::FixtureValues v;
        v.color = Utils::Colors::rgbToHsv(colors[i]); // frame stores colour as HSV
        frame.contribute(fids[i], v, Engine::MergePolicy::LTP);
    }
}
