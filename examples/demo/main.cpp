...
int main()
{
    ...
    auto rgb2 = lib.find("RGB").value();

    std::cout << rgb.describe();
    std::cout << rgb2.describe();

    for (int i = 0; i < 5; i++)
    {
        auto p = Fixture::createPtr(rgb);
        std::cout << p->UUID() << p->describe() << std::endl;
    }
    std::cout << rgb.describe();
