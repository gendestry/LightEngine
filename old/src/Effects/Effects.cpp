...
namespace
{
Utils::Colors::RGB lerp(const Utils::Colors::RGB &a,
                        const Utils::Colors::RGB &b, float t)
{
    auto mix = [t](uint8_t x, uint8_t y)
    {
        return static_cast<uint8_t>(
            x + (static_cast<float>(y) - static_cast<float>(x)) * t);
    };
...
// ---- DimmerChase ----
DimmerChase::DimmerChase(DMX::FixtureGroup g, Utils::Maths::Type type,
                         float bpm, float spread, uint16_t resolution)
    : Effect(std::move(g)),
      m_curve(Utils::Maths::getCurveByType(type, resolution)), m_type(type),
      m_bpm(bpm), m_spread(spread)
{
}
